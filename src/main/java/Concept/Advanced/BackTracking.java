...
public class BackTracking {
    ...
    private static void NineKeyPhasesBT(int[] keypress, Set<String> validWordsSet, Map<Integer, List<Character>> keysMap, int curIndex, List<String> curWords, String leftOverLetters, List<List<String>> allResults) {
        if (curIndex == keypress.length) {
            ...
            for (Character ch : choices) {
                // apply choice
                String tempWord = leftOverLetters + ch;

                // add new option
                if (validWordsSet.contains(tempWord)) {
                    // apply choice
                    curWords.add(tempWord);
                    NineKeyPhasesBT(keypress, validWordsSet, keysMap, curIndex + 1, curWords, "", allResults);
                    // undo choice
                    curWords.remove(curWords.size() - 1);
                }

                NineKeyPhasesBT(keypress, validWordsSet, keysMap, curIndex + 1, curWords, tempWord, allResults);
                // undo choice
            }
        }
    }
